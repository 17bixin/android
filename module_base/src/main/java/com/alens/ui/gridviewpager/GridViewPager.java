...
public class GridViewPager extends RelativeLayout {
     ...
     * necessary 必须作为最后一步
     */
    private List<GridViewAdapter> adapterList = null;
    public GridViewPager init(List<Model> list) {
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            final GridViewAdapter adapter = new GridViewAdapter(mContext, mData, i, pageSize);
        ...
        mPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            public void onPageSelected(int position) {
                ...
                for(int i = 0; i < adapterList.size(); i++){
                    GridViewAdapter adapter = adapterList.get(i);
                    adapter.setSelect(-1);
                    adapter.notifyDataSetChanged();
                }
