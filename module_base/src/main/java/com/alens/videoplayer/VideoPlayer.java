...
public class VideoPlayer extends FrameLayout implements View.OnClickListener,
    ...
    private boolean autoPlay, showControl;
    ...
    public VideoPlayer(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        showControl = a.getBoolean(R.styleable.VideoPlayer_showControl, false);
        a.recycle();
    ...
    private void init() {
        ...
        ViewGroup.LayoutParams params = controlLL.getLayoutParams();
        params.height = showControl ? 100:0;
        controlLL.setLayoutParams(params);
        mSurfaceView = view.findViewById(R.id.surfaceView);
        ivPlay = view.findViewById(R.id.ivPlay);
        ivAdjust = view.findViewById(R.id.ivAdjust);
        tvAllTime = view.findViewById(R.id.tvAllTime);
        tvPlayTime = view.findViewById(R.id.tvPlayTime);
        mSeekBar = view.findViewById(R.id.progress);
        tvSystemTime = view.findViewById(R.id.tv_system_time);
        tvTitle = view.findViewById(R.id.tv_title);
        topMenu = view.findViewById(R.id.rl_top_menu);
        playerScreen = view.findViewById(R.id.player_rl_screen);
        playerLock = view.findViewById(R.id.player_iv_lock);
        playError = view.findViewById(R.id.player_ll_error);
        netError = view.findViewById(R.id.player_ll_net);
        mProgressBar = view.findViewById(R.id.player_progressBar);
        ivBattery = view.findViewById(R.id.iv_battery);
        ivPlayCenter = view.findViewById(R.id.player_iv_play_center);
...
//                    ivPlay.setImageResource(R.drawable.video_pause);
        ...
        } else if (i == R.id.player_ll_error || i == R.id.player_ll_net) {
            //播放
            playVideo(videoPath, videoTitle, 0);
        }else if(i == R.id.player_iv_play_center){
            startVideo();
        }
    }
            ...
            try {
                if (mediaPlayer != null && mediaPlayer.isPlaying()) {
                    if (Math.abs(distanceX) > Math.abs(distanceY)) {// 横向移动大于纵向移动
                        if (distanceX >= PlayerUtil.dip2px(context, STEP_PROGRESS)) {// 快退，用步长控制改变速度，可微调
//                            gesture_iv_progress.setImageResource(R.drawable.player_backward);
                        ...
                        } else if (distanceX <= -PlayerUtil.dip2px(context, STEP_PROGRESS)) {// 快进
...
//                    gesture_iv_player_volume.setImageResource(R.drawable.player_volume_close);
                ...
                if (distanceY >= PlayerUtil.dip2px(context, STEP_VOLUME)) {// 音量调大,注意横屏时的坐标体系,尽管左上角是原点，但横向向上滑动时distanceY为正
...
//                    gesture_iv_player_volume.setImageResource(R.drawable.player_volume_open);
                } else if (distanceY <= -PlayerUtil.dip2px(context, STEP_VOLUME)) {// 音量调小
                    if (currentVolume > 0) {
                        currentVolume--;
                        if (currentVolume == 0) {// 静音，设定静音独有的图片
//                            gesture_iv_player_volume.setImageResource(R.drawable.player_volume_close);
        ...
        //调节亮度
            ...
            if (Math.abs(distanceY) > Math.abs(distanceX)) {// 纵向移动大于横向移动
                ...
                if (mLight >= 0 && mLight <= 255) {
                    if (distanceY >= PlayerUtil.dip2px(context, STEP_LIGHT)) {
                        if (mLight > 245) {
...
//        ivPlay.setImageResource(R.drawable.video_pause);
    ...
    public void startVideo() {
        if (mediaPlayer != null) {
            ivPlayCenter.setVisibility(GONE);
            mediaPlayer.start();
//            ivPlay.setImageResource(R.drawable.video_pause);
        }
    }
    ...
    public void pauseVideo() {
        if (mediaPlayer != null) {
            ivPlayCenter.setVisibility(VISIBLE);
            mediaPlayer.pause();
//            ivPlay.setImageResource(R.drawable.video_play);
            video_position = mediaPlayer.getCurrentPosition();
        }
